...
namespace PROKARYOTIC
{
    ...
    struct unique_id
    {
        ...
        unique_id(std::uint16_t min, std::uint16_t maj, std::uint16_t gr, std::uint16_t cl)
            :MIN(min), MAJ(maj), GROUP(gr), TYPE(cl) {}
	...
	public:
         ...
         : m_ID(0)
        {
            ...
            m_ID.TYPE  = static_cast<std::uint16_t>(obj_type);
        }
                    virtual unique_id  get_render_object_ID()   const { return m_ID;  }
        virtual e_RENDER_OBJECT_TYPES  get_render_object_TYPE() const { return static_cast<e_RENDER_OBJECT_TYPES>(m_ID.TYPE); }
